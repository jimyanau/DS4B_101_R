...
sp_500_daily_returns_tbl <- sp_500_prices_tbl %>%
    ...
    ungroup() %>%
    filter(!is.na(lag_1)) %>%
    mutate(adj_diff = adjusted - lag_1) %>%
    mutate(pct_return = adj_diff / adjusted_lag_1) %>% 
...
kmeans_obj$tot.withinss %>% glimpse()
```
...
umap_kmeans_results_tbl <- k_means_obj %>%
    ...
    left_join(umap_results_tbl, by = ("symbol"="symbol")) %>%
    left_join(sp_500_index_tbl %>% select(symbol, company, sector), by = c("symbol" = "symbol"))
...
umap_kmeans_results_tbl %>%
    ...
    scale_color_tq() +
    labs(
        title = "Cluster or Daily Returns for S&P 500 Index"
        , subtitle = "Color = Cluster"
        , color = "Cluster"
    )
