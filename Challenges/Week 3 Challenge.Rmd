...
bike_orderlines_tbl %>%
    ...
    mutate(black_inc = model %>% str_to_lower() %>% str_detect(pattern = "black inc")) %>%
    group_by(black_inc) %>%
    summarize(median_sales = median(total_price) %>% scales::dollar()) %>%
    ungroup() %>%
    set_names("Black Inc.", "Median Sales")
...
bike_orderlines_tbl %>%
    select(model, total_price) %>%
    mutate(ultegra_flag = model %>% str_to_lower() %>% str_detect(pattern = "ultegra")) %>%
    group_by(ultegra_flag) %>%
    summarize(median_sales = median(total_price) %>% scales::dollar()) %>%
    ungroup() %>%
    set_names("Ultegra", "Median Sales")
...
bike_orderlines_tbl %>%
    select(model, total_price) %>%
    mutate(disc_flag = model %>% str_to_lower() %>% str_detect(pattern = "disc")) %>%
    group_by(disc_flag) %>%
    summarize(median_sales = median(total_price) %>% scales::dollar()) %>%
    ungroup() %>%
    set_names("Disc", "Median Sales")
...
bike_orderlines_tbl %>%
    ...
    summarize(
        mean_price = mean(price) %>% scales::dollar(),
        min_price  = min(price) %>% scales::dollar(),
        max_price = max(price) %>% scales::dollar()
    ) %>%
    ungroup() %>%
    arrange(desc(mean_price)) %>%
    #set_names("Category 1", "Category 2", "Model Base", "Mean Price", "Min Price", "Max Price")
    # functional method
